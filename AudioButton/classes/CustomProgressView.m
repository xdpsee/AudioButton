...
- (id)initWithFrame:(CGRect)frame backColor:(UIColor *)aBackColor progressColor:(UIColor *)aProgressColor lineWidth:(CGFloat)aLineWidth progressShape:(CustomProgressShape)progressShape{
    ...
    if (self) {
        ...
        if (progressShape == CustomProgressShapeCircle) {
            CGPoint center = CGPointMake(CGRectGetWidth(self.frame) / 2, CGRectGetHeight(self.frame) / 2);
            const CGFloat radius = CGRectGetWidth(self.bounds) / 2 - lineWidth / 2;
        ...
        else if(progressShape == CustomProgressShapeRoundRectangle){
            const CGFloat tempWidth = self.frame.size.width - lineWidth;
            const CGFloat tempHeight = self.frame.size.height - lineWidth;
